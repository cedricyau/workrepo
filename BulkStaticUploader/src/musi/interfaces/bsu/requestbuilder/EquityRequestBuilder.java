...
public class EquityRequestBuilder extends RequestBuilder {
	private String requestsDir;
	private Logger log = Logger.getLogger(getClass());

	public EquityRequestBuilder(String requestsDir, String securityFile, String fieldFile) {
		...
		try {
			...
			File[] currentDirList = outputLocation.listFiles(new FilenameFilter() {
				public boolean accept(File dir, String name) {
					return name.startsWith("EQUITY_");
				}
			});
			...
			try {
				for (Security security : securities) {
					...
					osw = new OutputStreamWriter(new FileOutputStream(new File(requestsDir + File.separator + "EQUITY_" + securityType + "_" + securityId + ".xml")));
